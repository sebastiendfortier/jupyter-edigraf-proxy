...
def _edigraf_urlparams():
    from getpass import getuser

    url_params = '?' + '&'.join([
        'username=' + getuser(),
        'password=' + _edigraf_passwd,
        'encryption=AES',
        'key=' + _edigraf_aeskey,
        'sharing=true',
    ])
...
def _edigraf_mappath(path):

    # always pass the url parameter
    if path in ('/', '/index.html', ):
        url_params = _edigraf_urlparams()
...
def setup_edigraf():
    ...
    global _edigraf_passwd, _edigraf_aeskey
    ...
    _edigraf_aeskey = _get_random_alphanumeric_string(16)
    try:
    ...
    path_info = 'edigraf/index.html' + _edigraf_urlparams()
    ...
    return {
        ...
        'command': cmd,
        'mappath': _edigraf_mappath,
        'absolute_url': False,
        'timeout': 90,
        'new_browser_tab': True,
        'launcher_entry': {
            'enabled': True,
            'icon_path': os.path.join(HERE, 'icons/edigraf-logo.svg'),
            'title': 'Edigraf',
            'path_info': path_info,
        },
    }
