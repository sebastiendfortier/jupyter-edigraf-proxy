![build](https://github.com/sebastiendfortier/jupyter-edigraf-proxy/workflows/build/badge.svg)

# jupyter-edigraf-proxy
...
(incl. [jupyter-server-proxy](https://github.com/jupyterhub/jupyter-server-proxy), [jupyter-edigraf-proxy](https://github.com/sebastiendfortier/jupyter-edigraf-proxy) and [Xpra-HTML5](https://xpra.org/))
...
- you use jupyter-edigraf-proxy with a version number >= 0.3.0 (not on pypi)
...
#### Install jupyter-edigraf-proxy
```
pip install git+https://github.com/sebastiendfortier/jupyter-edigraf-proxy.git
```

#### Enable jupyter-edigraf-proxy Extensions
...
You have to modify `setup_edigraf()` in `jupyter_edigraf_proxy/__init__.py` for change.
